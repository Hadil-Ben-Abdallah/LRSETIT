...
import "./HabilitationsTable.css";
import { Link, useParams } from "react-router-dom";
import "bootstrap-icons/font/bootstrap-icons.css";

function HabilitationsTable() {
  ...
  useEffect(() => {
    axios.get("http://localhost:8081/new_habilitations")
      .then((res) => {
  ...
  function handleDelete(id) {
    axios.delete(`http://localhost:8081/delete_habilitation/${id}`)
      .then((res) => {
    ...
    <div className="card-body d-flex justify-content-center align-items-center">
      <div className="table-responsive">
        <table className="table proj">
          <thead>
            <tr>
              <th className="text-center" onClick={() => handleSort("annéeHabi")}>
                <div>Année</div>
                <div><i className="fas fa-sort"></i></div>
              </th>
              <th className="text-center" onClick={() => handleSort("titreHabi")}>
                <div>Titre</div>
                <div><i className="fas fa-sort"></i></div>
              </th>
              <th className="text-center" onClick={() => handleSort("nomHabi")}>
                <div>Nom et prénom titulaire habilitation</div>
                <div><i className="fas fa-sort"></i></div>
              </th>
              <th className="text-center" onClick={() => handleSort("fileHabi")}>
                <div>Fichier</div>
                <div><i className="fas fa-sort"></i></div>
              </th>
              <th className="text-center" onClick={() => handleSort("encadrantHabi")}>
                <div>Encadrant</div>
                <div><i className="fas fa-sort"></i></div>
              </th>
              <th className="text-center" onClick={() => handleSort("dateHabi")}>
          ...
          <tbody>
            {sortedData.map((habilitation) => (
              <tr key={habilitation.id}>
                <td>{habilitation.annéeHabi}</td>
                <td>{habilitation.titreHabi}</td>
                <td>{habilitation.nomHabi}</td>
                <td>{habilitation.fileHabi}</td>
